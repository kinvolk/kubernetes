...
func TestUserNsManagerAllocate(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, 65536, int(length), "m.isSet(%d).length=%v", allocated, length)
	assert.Equal(t, true, m.isSet(allocated), "m.isSet(%d)", allocated)
	assert.Equal(t, 65536*2, int(allocated))
	...
	assert.Equal(t, uint32(65536*3), allocated2)
	...
	var allocs []uint32
	for i := 0; i < 1000; i++ {
		allocated, length, err = m.allocateOne(fmt.Sprintf("%d", i))
		assert.Equal(t, 65536, int(length), "length is not the expected. iter: %v", i)
		assert.Equal(t, 65536*(i+2), int(allocated), "firstID is not the expected. iter: %v", i)
		assert.NoError(t, err)
		allocs = append(allocs, allocated)
	}
	for i, v := range allocs {
		assert.Equal(t, true, m.isSet(v), "m.isSet(%d) should be true", v)
		m.Release(fmt.Sprintf("%d", i))
		assert.Equal(t, false, m.isSet(v), "m.isSet(%d) should be false", v)

		err = m.record(types.UID(fmt.Sprintf("%d", i)), v, 65536)
		assert.NoError(t, err)
		m.Release(fmt.Sprintf("%d", i))
		assert.Equal(t, false, m.isSet(v), "m.isSet(%d) should be false", v)
	}
...
func TestUserNsManagerHostIDFromMapping(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if (tc.success && err != nil) || (!tc.success && err == nil) {
				t.Errorf("%v: expected success: %v - got error: %v", tc.name, tc.success, err)
				return
			}
...
func BenchmarkBitmaskFindAndSetFirstZero(t *testing.B) {
	b := makeBitArray(65536)
	for i := 0; i < 65536; i++ {
		_, err := b.findAndSetFirstZero()
		assert.NoError(t, err)
	}
}
