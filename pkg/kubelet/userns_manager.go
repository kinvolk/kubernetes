...
func (m *usernsManager) allocateOne(pod string) (firstID uint32, length uint32, err error) {
	if m.currPods >= maxPods {
		return 0, 0, fmt.Errorf("limit on count of pods with user namespaces exceeded (limit is %v, current pods with userns: %v)", maxPods, m.currPods)
	}
	m.currPods++
	defer func() {
		if err != nil {
...
func (m *usernsManager) record(pod types.UID, from, length uint32) (err error) {
	...
	if m.currPods >= maxPods {
		return fmt.Errorf("limit on count of pods with user namespaces exceeded (limit is %v, current pods with userns: %v)", maxPods, m.currPods)
	}
	m.currPods++
	defer func() {
		if err != nil {
...
func (m *usernsManager) releaseWithLock(podUID types.UID) {
	pod := string(podUID)
	v, ok := m.usedBy[pod]
	if !ok {
		klog.V(5).InfoS("pod user namespace allocation not present", "podUID", podUID)
		return
	}
	delete(m.usedBy, pod)

	klog.V(5).InfoS("releasing pod user namespace allocation", "podUID", podUID)
	m.currPods--
	m.removed++
...
func (m *usernsManager) CleanupOrphanedPodUsernsAllocations(pods []*v1.Pod, runningPods []*kubecontainer.Pod) error {
	...
	for podUID := range m.usedBy {
		if allFound.Has(podUID) {
			continue
		}

		klog.V(5).InfoS("Clean up orphaned pod user namespace possible allocation", "podUID", podUID)
		m.releaseWithLock(types.UID(podUID))
	}
