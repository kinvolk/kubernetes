...
func (m *kubeGenericRuntimeManager) waitForSidecars(pod *v1.Pod, podStatus *kubecontainer.PodStatus, changes *podActions) bool {
	...
	if len(sidecars) == 0 || len(rest) == 0 {
		klog.V(4).Infof("No sidecars or all containers are sidecars for pod: %q. Continuing with the main loop", format.Pod(pod))
		return false
	}
	...
	if restStarted {
		klog.V(5).Infof("Non-sidecar containers already started for pod: %q. Continuing with the main loop", format.Pod(pod))
		return false
	}
	...
	// XXX: review if we want to return also when there are containers to
	// kill or not
	...
	if !allReady {
		klog.V(5).Infof("Sidecars for pod %q not yet ready, waiting for them to be ready", format.Pod(pod))
		return true
	}
...
func (m *kubeGenericRuntimeManager) computePodActions(pod *v1.Pod, podStatus *kubecontainer.PodStatus) podActions {
	...
	if wait := m.waitForSidecars(pod, podStatus, &changes); wait {
		klog.V(4).Infof("Waiting for sidecars in pod: %q", format.Pod(pod))
		return changes
	}
...
func (m *kubeGenericRuntimeManager) killSidecars(pod *v1.Pod, podStatus *kubecontainer.PodStatus, changes *podActions) {
	...
	if pod.Spec.RestartPolicy == v1.RestartPolicyAlways {
		klog.V(5).Infof("Not killing sidecars for pod: %q, pod.Spec.RestartPolicy is Always", format.Pod(pod))
		return
	}
	...
	if len(sidecars) == 0 {
		klog.V(5).Infof("No sidecar running for pod: %q, not killing sidecars", format.Pod(pod))
		return
	}
	...
	if !nonSidecarsNotRunning {
		klog.V(5).Infof("Found non-sidecar containers running for pod %q. Won't proceed to kill sidecars", format.Pod(pod))
		return
	}
	...
	for idx, container := range pod.Spec.Containers {
		...
		// running.
		if containerStatus == nil || containerStatus.State != kubecontainer.ContainerStateRunning {
			klog.V(5).Infof("Not killing container that is not running: %q pod: %q", container.Name, format.Pod(pod))
			continue
		}
...
func (m *kubeGenericRuntimeManager) SyncPod(pod *v1.Pod, podStatus *kubecontainer.PodStatus, pullSecrets []v1.Secret, backOff *flowcontrol.Backoff) (result kubecontainer.PodSyncResult) {
	fmt.Printf("kubeGenericRuntimeManager SyncPod: Called\n")
	...
	if podContainerChanges.KillPod {
		...
		fmt.Printf("kubeGenericRuntimeManager SyncPod: 1 - add killResult %v\n", killResult)
		...
		for containerID, containerInfo := range podContainerChanges.ContainersToKill {
			...
			fmt.Printf("kubeGenericRuntimeManager SyncPod: 2 - add killContainerResult %v\n", killContainerResult)
	...
	if podContainerChanges.CreateSandbox {
		...
		fmt.Printf("kubeGenericRuntimeManager SyncPod: 3 - createSandboxResult %v\n", createSandboxResult)
		...
		if err != nil {
			...
			fmt.Printf("kubeGenericRuntimeManager SyncPod: 4 - err %v\n", err)
			result.Fail(err)
			return
		}
	...
	fmt.Printf("kubeGenericRuntimeManager SyncPod: 5 - AddSyncResult %v\n", configPodSandboxResult)
	...
	start := func(typeName string, container *v1.Container) error {
		...
		fmt.Printf("kubeGenericRuntimeManager SyncPod: 6 - startContainerResult %v\n", startContainerResult)
